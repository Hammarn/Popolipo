# Popolipo Installation

To start using the Popolipo pipeline, there are three steps described below:
...
This pipeline itself needs no installation - NextFlow will automatically fetch it from GitHub if `Popolipo` is specified as the pipeline name.
...
```bash
wget https://github.com/Popolipo/archive/master.zip
unzip master.zip -d /my-pipelines/
cd /my_data/
nextflow run /my-pipelines/popolipo-master
```
