...
data_dir=${data_path}/popolipo_test_set
...
then
    ...
    curl https://your_website.com/popolipo_test_set.tar.bz2 > ${data_path}/popolipo_test_set.tar.bz2
    echo "Unpacking test set..."
    tar xvjf ${data_path}/popolipo_test_set.tar.bz2 -C ${data_path}
    echo "Done"
fi

# Run name
run_name="Test Popolipo Run: "$(date +%s)
